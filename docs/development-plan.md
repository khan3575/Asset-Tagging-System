...
- **2026-08-27: Stages 2, 3, 5, and 6 of the Refactoring Roadmap are done. Stage 7 (T7.1) is in progress, not finished — two real bugs found in it, not yet fixed.** Detail:
  ...
  - **Stage 6 — view parameters, roadmap T6.1–T6.2, done, plus one page (`audit-log.xhtml`) the roadmap didn't mention but had the identical pattern.** `f:viewParam`/`f:viewAction` now on `asset-view.xhtml`, `user-detail.xhtml`, `asset-list.xhtml`, `user-list.xhtml`, and `audit-log.xhtml`. `FacesUtil.java` is deleted — `grep -rn "getRequestParams" src/` returns nothing. `PageParams` was kept but its API changed from `parse(Map<String,String>, int)` to two small static methods, `clamp(Integer)`/`offset(int, int)`, since `f:viewParam` now does the binding `PageParams` used to do by hand.
    ...
    - Worth understanding, not just knowing: JSF has no path-variable mechanism of its own — `f:viewParam` only reads query-string parameters. A clean path like `/assets/1` can only exist because `AssetController` bridges it into `/asset-view.xhtml?id=1` first. The controller and `f:viewParam` are complementary layers, not competing/redundant ones — Stage 8 ("URL migration") is about the *sidebar's* outcome-string/filename mismatches, not about removing these controllers.
  - **Stage 7 (T7.1, user edit flow) — in progress, not finished.** `UserDao.updateUser(...)`/`replaceRoles(...)` and a new `UserService.updateUser(...)` exist. `UserDetailBean` is `@ViewScoped` and injects `UserService`. **Not yet done**: the `editing` state machine (`edit()`/`save()`/`cancel()` methods, no `immediate="true"` handling for Cancel yet), and `user-detail.xhtml` still has the old JS-toggle (`user-detail.js`) `<form>`, not `h:form` with dual-mode fields. **Two real bugs sitting in the code right now, found but not yet fixed:**
    1. `UserService.updateUser(...)` is annotated `@jakarta.transaction.Transactional`, not `org.springframework.transaction.annotation.Transactional`. This app has no JTA transaction manager configured (confirmed by its own boot log: `WELD-000101: Transactional services not available`) — Spring's AOP transaction interception is wired for its own annotation, so this is very likely a silent no-op, meaning `updateUser`'s two DAO calls (`updateUser` + `replaceRoles`) are *not* actually sharing one transaction. Needs the same fault-injection proof as Stage 2's `AssetService.register` before trusting it, once it's fixed.
    2. `UserService.getUser(id)` returns `userDao.findById(id).orElse(null)` — silently `null` for a nonexistent id, instead of the established `OptionalUtils.orThrowDbFetch(...)` pattern every other detail bean uses (`AssetDetailBean`, the old `UserDetailBean`). Visiting `/user/99999` would now silently set `user = null` rather than a clean not-found error, and later `user.firstName`-style EL would just render blank rather than surfacing the problem.
    - Also outstanding, called out at design time rather than found as a bug: `ats:filterSelect` (built for GET-based list-filter pages reading state via `f:viewParam`) is a plain literal `<select>` with no JSF value binding — it can't be reused for the edit-mode department/status dropdowns inside a real `h:form` postback. Those need `h:selectOneMenu` (mirroring `add-asset.xhtml`'s category dropdown pattern) instead. Similarly, role editing should use `h:selectBooleanCheckbox` (one per role — there are only two), not the current plain, unbound `<input type="checkbox">`.
...
- **What's next: finish Stage 7's T7.1 first — it's mid-flight, not a fresh start.** In order: (1) fix the two bugs noted above (`@jakarta.transaction.Transactional` → `@org.springframework.transaction.annotation.Transactional`; `getUser` → `OptionalUtils.orThrowDbFetch`), (2) build the `editing` state machine (`edit()`/`save()`/`cancel()`, `immediate="true"` on Cancel) on `UserDetailBean`, (3) convert `user-detail.xhtml` to `h:form` with dual-mode fields (`h:selectOneMenu` for department/status, `h:selectBooleanCheckbox` per role — not `ats:filterSelect`, see the note above), (4) delete `user-detail.js`. Then roadmap T7.2 (convert the two search forms to `h:form` + `faces-redirect=true&includeViewParams=true`, so filtered URLs stay bookmarkable). This supersedes the previous "Step 2 → Step 4 → Step 7" ordering below, which predates the roadmap.
...
**Superseded by roadmap Stage 7, T7.1 — in progress, not finished.** List and detail pages exist and work. The edit flow is being rebuilt now: see the 2026-08-27 status note above for exactly what's done, what's left, and the two bugs found in the parts already written. The old plan below (a raw `POST /user/{id}` handler) is obsolete — T7.1 uses `@ViewScoped` + `h:form` + `f:viewAction`/command-button actions instead, not a hand-written controller endpoint.
...
- [ ] Finish T7.1 (see the 2026-08-27 status note's numbered list above)
- [ ] Once T7.1's `save()` actually persists, wire its activity-log write the same way `AssetService.register` does — inside `UserService.updateUser`'s `@Transactional` method, not a standalone call
